...
impl KernelFrameAllocator {
            ...
            .filter(|x| {
                self.frame_tracker.as_ref().iter().all(|frame| {
                    !(frame.frame.start_address()
                        ..frame.frame.start_address() + frame.count.get() as u64 * 4096)
    ...
    // pub fn allocate_frames_ty(
    //     &mut self,
    //     frames: &mut [MaybeUninit<PhysFrame<Size4KiB>>],
    //     ty: FrameUsageType,
    // ) -> usize {
    ...
    //         self.frame_tracker.push_used_frame(UsedFrame { frame, count: NonZero::new(1).unwrap(), ty });
    //     }
        ...
        for frame in &frames[0..frames_allocated] {
            ...
            match curr {
                Some(ref mut current) => {
                    let next_expected_addr =
                        current.frame.start_address() + current.count.get() as u64 * 4096;
                    if frame.start_address() == next_expected_addr {
                        current.count = current.count.checked_add(1).unwrap();
                    } else {
